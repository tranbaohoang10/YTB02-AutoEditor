...
class CheckContractTests(unittest.TestCase):
    def test_check_requires_script_and_runs_mode_aware_dry_validation(self) -> None:
        ...
        self.assertIn("[fail] input\\script.en.json is missing", content)
        self.assertIn("[fail] input\\script.vi.json is missing", content)
        self.assertIn("set \"check_failed=1\"", content)
        self.assertIn(
            "-m src.pipeline --script input\\script.en.json --config config.json --dry-run",
            content,
        )
        self.assertIn(
            "-m src.pipeline --script input\\script.vi.json --config config.json --dry-run",
            content,
        )
        self.assertIn("english script and referenced visual sources are valid", content)
        self.assertIn("vietnamese script and referenced visual sources are valid", content)
