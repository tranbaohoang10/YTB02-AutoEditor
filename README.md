...
- Logo sparkle Gemini/Flow đã biết được xóa deterministic ở tầng prepared scene bằng alpha envelope đo từ clip thật và selector cục bộ theo từng frame. Khung sạch được giữ nguyên pixel; khung có mark dùng deblend hoặc patch texture xác định trong envelope, không median-smear, không paper rectangle và không crop/zoom toàn frame. `safe_edge_crop`/paper patch chỉ còn là lựa chọn explicit. Final channel bug burn đúng một lần sau subtitle/transition và chỉ dùng composite PNG `l0ki_archives_logo.png`; pipeline không vẽ thêm wordmark hay text phụ.
...
Các section `source_cleanup`, `visual_quality`, `transitions`, `subtitles` và `watermark` trong `config.json` giữ precise sparkle ROI/alpha envelope, fallback explicit, profile threshold, timing/gain và final style. Strategy production mặc định `cover_with_official_logo` bỏ qua hoàn toàn OpenCV reconstruction/cache ở Step 7 rồi dùng đúng `assets/branding/l0ki_archives_logo.png` một lần trong final composition. Logo giữ aspect ratio ở width 110px, opacity 0.42, base safe margin phải/dưới 14px và nudge 111px sang trái/112px lên trên để nằm đúng measured Gemini location; không có background card/badge. Logo được composite trước ASS để subtitle không bị che và final watermark 76px không được burn lần hai. Khi cần fallback, chọn explicit `frequency_selective_reconstruct` để chạy stream frame cục bộ trước visual-quality và cache kết quả cleanup. Chỉ scene có subtitle-band density cao mới tăng outline cho current karaoke text; ASS shadow vẫn tắt và `\ko` không làm lộ fill/outline/shadow của từ tương lai trước timestamp alignment. Không có `drawtext`, wordmark phụ, asset branding mới hay thay đổi PNG chính thức.
